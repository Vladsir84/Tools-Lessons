import { createTemporaryCell } from './temporary-cell';
...
export const addButton = (event) => {
  if (event.target.classList.value === 'emptyCell') {
    ...
    const month = document.querySelector('.dates').innerHTML.split(' ')[0];
    const startMonth = '01';

    const year = document.querySelector('.dates').innerHTML.split(' ')[1];
    const startYear = year;

    const startDay = event.target.dataset.date;
