import { arrOfEvents } from './storage';
...
export const checkEvent = () => {
  ...
  const newEventStart = new Date(`${startDate.value}T${startTime.value}`);
  const newEventEnd = new Date(`${endDate.value}T${endTime.value}`);
  let selectedId;


  for (let i = 0; i < arrOfEvents.length - 1; i++) {
    const eventStart = new Date(arrOfEvents[i].startDate);
    const eventEnd = new Date(arrOfEvents[i].endDate);
...
export function validate(start, end) {
  if (!errorDate(start, end) || !duration(start, end) || !checkForUpdate(start)
        || !checkEvent()) return false;
}
